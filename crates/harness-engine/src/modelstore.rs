...
///
/// 以前这里是 `std::fs::read(p)` + `Sha256::digest(&data)`：对一个 456MB 的权重文件，
/// 那等于每次自检都（1）把 456MB 读进内存（内存尖峰 + 页缓存冲刷），（2）重算一遍全量哈希。
/// 实测这一下发要 **6.5 秒**；如果调用方又是**同步命令**，主线程就被占满 —— 界面里在飞的
/// IPC 回复送不回去，表现成"界面永远挂起"。所以这里流式读、并且**每份文件只真校验一次**。
