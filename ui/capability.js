...
(function () {
  ...
  async function addTool() {
    ...
    if (!name) return status("请填写工具名", "error");
    try {
  ...
  async function saveSkill() {
    ...
    if (!name) return status("请填写技能名", "error");
    try {
      ...
      status("保存失败: " + err, "error");
    }
  }

  async function deleteSkill() {
    const invoke = getInvoke();
    if (!invoke || !selectedSkill) return status("请先在列表中选择技能", "error");
    try {
      await invoke("skills_remove", { name: selectedSkill, projectRoot: root() });
      status("技能已删除: " + selectedSkill);
      newSkill();
      await refreshSkills();
    } catch (err) {
      status("删除失败: " + err, "error");
  ...
  async function toolsCommand(rest) {
    ...
    if (args[0] === "add") {
      ...
      const name = args[1];
      if (!name) return status("用法: tools add <名称> [命令] [描述]", "error");
      try {
        await invoke("tools_add", {
          name,
          command: args[2] || null,
    ...
    if (args[0] === "probe") {
      if (!args[1]) return status("用法: tools probe <名称>", "error");
      return probeTool(args[1]);
    }
    if (args[0] === "remove" || args[0] === "rm") {
      if (!args[1]) return status("用法: tools remove <名称>", "error");
      return removeTool(args[1]);
    }
    status("用法: tools [list | add <名> [命令] [描述] | probe <名> | remove <名>]", "error");
  }
  ...
  async function skillsCommand(rest) {
    ...
    status("技能编辑请用左侧「技能」面板（skills.toml 同步生效）", "error");
  }
