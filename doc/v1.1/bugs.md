...
> **2026-09-26 补**：本 ISSUE 涉及的语音转写功能**已整体删除**（用户拍板）；这里保留是因为那个根因与修法**与语音无关** —— 「昂贵的自检要缓存，且绝不许放在主线程」对所有模型自检都成立。删除记录见 `doc/v1.1/需求-本地语音转写-v1.1-已删除.md`。
...
## ISSUE-8：96 轮 207 次调用、0 文件变更 —— 折叠后的历史在字面上教模型重读

**状态**：**待实施**（方案已拍板 2026-09-26；见 [需求-Agent-进展记忆与循环守卫-v1.1.md](需求-Agent-进展记忆与循环守卫-v1.1.md)）。

### 症状（可复现）

问一句**诊断题**（「为什么配置模型后，会话 UI 里却一直是模型未知？难道是 anthropic 兼容接口无法
探测可用模型？」），会话跑到轮数上限才停：

- **96 轮**（上限）/ **207 次**工具调用 / **0 个文件变更** / 没有结论；
- 反复 `read ui/scripts/session.js`（第 93、95 轮…）、反复 `findstr` 同一批符号；
- 每一轮都有一行 `历史折叠 1 轮（省 236 字节；保留最近 6 轮正文）` —— **连 236 字节都折**；
- 结束语：`（达到 96 轮上限，循环停止。已完成 207 次工具调用、0 个文件变更；请基于以上进展继续指示。）`

### 根因

`crates/harness-engine/src/agent.rs::fold_history` 把保留窗口外的每一轮换成两个摘要：
`call_shape`（**留下 path + range**）与 `outcome_line`（**只留输出首行**，截 100 字符），
而摘要的**原文**是：

```
…正文已从上下文移除，需要时重新 read
```

对「答案就在文件正文里」的诊断题：模型每读一次、六轮后内容被折掉、只剩「我读过它」，
于是再读一遍 —— **闭环**。

**口径**：引擎不是失忆，它记的是「做了什么」，丢的是「知道了什么」；而那句摘要还在**教模型重读**。
`history_keep_rounds`（默认 6）**先不要调大** —— 那只是抬高天花板、token 线性上涨。

### 证据（真机）

1. trace（用户截图）：第 93/95/96 轮的 `read ui/scripts/session.js`、`findstr` 重复出现；
   折叠行每轮一条；
2. **题目本身有确定答案**（真应用 + CDP，与模型无关）：
...
⇒ 「模型未知」不是探测失败，而是前端取一次后**没有重取**（`session.js:302-313` 的两个值只在
打开/刷新时取）—— 一道 5 轮能答的题，96 轮没答出来；
3. 读代码：`fold_history` 全文与两个 `digest` 的构造处（`agent/tool_loop.rs:713-733`）。

### 改法

见需求文档 §4：**一份引擎账本**（引擎写、永不折叠）+ `record_findings`（模型写、永不折叠、
带 id 与 `supersede`）+ **重复读守卫** + **停滞守卫（K=8 ⇒ 强制 final）** + 折叠措辞反写。

### 门禁

**待实施**。按需求文档 §6.3 的八条引擎单测落地；主判据「重复读次数 ≤ 基线 50%，且 3 次里 ≥2 次
给出可用答复」，否定判据「findings 累计 > 上下文 1/3 ⇒ 判设计失败（哪怕主判据达标也不接受）」。
