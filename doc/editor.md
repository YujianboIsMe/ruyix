...
那一行同时喂两处排版炸弹：背板的 `.code-line`（28 万字符的文本节点；走高亮时还是十万级 span）
和 textarea 里同长度的单行原文。**纵向虚拟化对它无效** —— 它就是"可见行"，必然整行建 DOM。

只读视图做四件事，少任何一件都白改：

| # | 做法 | 为什么 |
|---|---|---|
| 1 | 超长行按**视觉列**切成显示段，段宽 = `min(200 列, 视口 − 行号列 − padding)` | 背板那一刻最多几百字符，不再有 28 万字符的文本节点。段宽必须**按视口算**：写死 200 列在 560px 视口下撑出 1587px 的横向滚动区（实测） |
| 2 | **textarea 退出布局**（`display:none`） | 它装着全文，留在流里就等于让 layout 去排版那条 28 万字符的单行（内部 scrollWidth 到 2.2e6 px）。必须 `display:none` —— `visibility:hidden` 仍会被排版 |
| 3 | 全文高度改由**流内 spacer** 给出（`#editor-flow-spacer`） | textarea 一走，`.editor-view` 就没有可滚的高度了（原本靠它撑） |
| 4 | 宽度占位（`.editor-virt-keeper`）**不给**，容器回到视口宽 | 撑到几百万像素的那个宽度正是病根之一 |

**行号规则**（用户拍板）：顶部一条**虚拟行**显示【行太宽，触发只读限制】，
它占一行高但**不进行号** —— gutter 第一格是空的、内容首行仍是 `1`。同一条规则顺带覆盖续格：
一条逻辑行占多格时只有**第一格**给号。实现上只存一个映射 `editorModel.lineOf`
（显示行 → 逻辑行，横幅记 `-1`），行号是从它**推导**出来的 —— 不要退回"另存一份'要不要编号'"，
那样显示段就拼不回原文了（v0.13 第一版正是错在这里，被 `scripts/editor-wide-line.js` 当场抓住）。

**其它约定**：

- 超阈值文件**不进 tree-sitter**（那一行约 10 万 token，着色收益低而代价高）→ 纯文本单色。
  出口是 `renderWideReadOnly`；判定只在 `setEditorContent` 里做一次（三个渲染入口都走它，见 C4），
  事实落在 `editorModel.wide`，标签图标从它抄（`noteWideReadOnly`）；
- 视口变化必须**重切段**（`recutWideModel`）：段宽是视口算的，窗口一窄旧段就比容器宽，
  而且显示行数与 spacer 高度也全错（滚不到最后几行）。素材是 `editorModel.rawLines`
  （原始逻辑行 —— 拿显示段回切不出原文）；
- 标签图标换成 🔒（`tabIcon`）：一眼看出"这个文件只是给你看，不给改"；
- 门禁：`scripts/editor-wide-line.js`（真浏览器，四个臂：阈值上下各一列 / 283,184 列 /
  视口变窄重切 / **反向验证**——把阈值调大后旧病征必须复现，否则这门是摆设）。
  ui-smoke 的 U47 转调它，本机没浏览器时 SKIP 并显式说明；
- **还没做**：可编辑的横向窗口（保留 textarea、只挂可见列的节点 + 背板跟随 textarea 横滚）。
  能不能做取决于一个尚未测的问题：那 28 万字符单行的**整形**本身是不是崩溃元凶。
  方案与预注册判据见 `doc/v0.x/需求-水平按需渲染-v0.13.md`。
