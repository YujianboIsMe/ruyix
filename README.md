...
- **只有单 exe 也能自检并取模型**：模型规格（文件名/体积/sha256/来源 URL）**编译进二进制**，
  所以 exe 自己知道该取什么。缺模型时启动即在后台取（进度写进状态栏与记忆面板），
  也可在面板里点〔取模型〕。`--no-fetch` 关掉自动取（绿色手动路线），`--fetch-model` 强制取一次。
  真源可复跑证明：`cargo run -q -p harness-engine --example model_fetch_probe`（只取 110KB 的小文件）。
